...
const Index = ({ title, data, id }: propsData) => {
  ...
  const { updateDataLessons, postDataLessons } = useLassonsStore();
  ...
  const handelSubmit = async (value: any) => {
    ...
    let status;
    if (!id) {
      status = await postDataLessons({...value, courseId: courseId});
    } else {
      const updateData = { ...data, ...value , courseId: courseId  };
      status = await updateDataLessons(updateData);
    }
          ...
          >
            ...
            <div>
              ...
              <Form.Item
                ...
                style={{ width: "100%" }}
                rules={[{ required: true }]}
                initialValue={data?.video ? data.video : ""}
              >
                <Input style={{ width: "100%" }} size="large" />
              </Form.Item>
            </div>
